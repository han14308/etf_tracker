...
def get_krx_backfill_status() -> dict:
    with _lock:
        return dict(_status)


def start_krx_backfill(days: int) -> bool:
    with _lock:
        if _status["running"]:
            return False
        _status.update(
    ...
    thread = threading.Thread(target=_run_krx_backfill, args=(days,), daemon=True)
    thread.start()
    return True
...
def _run_krx_backfill(days: int) -> None:
    total = 0
    try:
        for trade_date in recent_weekdays(days):
            ...
            time.sleep(1)
        _add_message(f"KRX backfill complete rows={total}")
    finally:
        with _lock:
